...
public class ArticleFreemarkerServiceImpl implements ArticleFreemarkerService {
    ...
    public void buildArticleToMinIO(ApArticle apArticle, String content) {
        ...
        if (StringUtils.isNoneBlank(content)){
            ...
            try {
                ...
                //数据模型
                Map<String,Object> contentDataModel = new HashMap<String,Object>();
    ...
    public void buildArticleToOSS(ApArticle apArticle, String content) {
        ...
        if (StringUtils.isNoneBlank(content)) {
            ...
            //4.3 把html文件上传到OSS中
            InputStream in = new ByteArrayInputStream(out.toString().getBytes());
            String path = aliyunFileStorageService.uploadHtmlFile("", apArticle.getId() + ".html", in,apArticle.getId() + ".html");
            //4.4 修改ap_article表，保存static_url字段
            apArticleService.update(Wrappers.<ApArticle>lambdaUpdate().eq(ApArticle::getId, apArticle.getId()).set(ApArticle::getStaticUrl, path));
            //发送消息，创建索引
            createArticleESIndex(apArticle, content, path);
